...
- `workspace` — array of local directories the employee may read and edit (e.g. `["/Users/me/Workspace"]`). Grants Read/Grep/Glob/Edit/Write plus Bash restricted to `git`. Rules enforced by prompt: code changes happen in a linked worktree under `~/.matilda-crew/worktrees/` on a dedicated `crew/<task-id>-<slug>` branch (your checkout is never touched and your workspace stays clean — inspect its branches from the repo with plain git), commits allowed, **push never**, and every code change is reported in the task comment with repo, branch and diff-stat. Omit the field for a Matilda-only employee (no filesystem at all).
