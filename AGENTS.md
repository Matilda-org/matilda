...
- API: contacts and campaigns are readable (plus contact creation and communication enqueuing), and `/api/v1/communications/:id` exposes a communication with its notes (`logs` GET/POST, `logs/:log_id` DELETE). Note content is rich text: `as_json(with_content: true)` opts into the HTML, same pattern as `Task`.
