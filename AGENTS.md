...
- Campaign show is a 4-column board (`campaigns/_kanban`, `c-comm-board*`, no drag) with a segmented distribution bar; each column reads in its own order (`Communication.board_order`: oldest queued/waiting first, latest outcomes on top) and cards show how long a sent one has been waiting (`days_waiting`, warning after 7 days, danger after 21).
- Board cards carry no menu: the whole card opens the detail overlay, which holds every action (send, outcome, follow-up, delete) plus the state timeline and the Follow-up / Note tabs. In the Note tab the Trix editor stays anchored outside the scroller (only the note list scrolls) — Trix inside a scrolling container makes the scroll glitch.
