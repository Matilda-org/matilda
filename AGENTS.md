...
- Campaign show is a 4-column kanban (`campaigns/_kanban`, reuses the `c-kanban__*` CSS, no drag): cards advance via modal actions (Invia / Esito); the detail overlay reuses the task-overlay pattern (single scroller, Follow-up / Note tabs). Editing follow-ups or notes answers with turbo streams replacing both the `action` frame (the overlay stays open on the tab the action came from, via `@active_tab`) and the `campaign-kanban` frame.
